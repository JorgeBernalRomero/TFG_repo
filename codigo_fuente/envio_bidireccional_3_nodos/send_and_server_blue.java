...
public class send_and_server {
	...
	public static void listeningFromGreen (Session session){
		try{
			...
			if (msg instanceof MapMessage) {
				...
				String saveDirectory = "/root/TFG/recepcionResults/greenResults"; //esta parte se podría sustituir pasando el dirDestino por parámetro
	    ...
	    //Function which makes node to listen and await for the income message
		public static void listeningFromPink (Session session){
			try{
				MessageConsumer consumer = session.createConsumer(session.createQueue("domibus.backend.jms.outQueue"));
				Message msg = consumer.receive(); //podríamos poner 10000 que serían 10 segundos
				System.out.println(msg);
				...
				if (msg instanceof MapMessage) {
					...
					String saveDirectory = "/root/TFG/recepcionResults/pinkResults"; //esta parte se podría sustituir pasando el dirDestino por parámetro
					File saveDir = new File(saveDirectory);
					if (!saveDir.exists()) {
						saveDir.mkdirs();
					}
    ...
    public static void sendingToGreen(Session session){
		try{
			...
			messageMap.setStringProperty("toPartyId", "domibus-green"); //nodo destino
            ...
            File file = new File("/root/TFG/envioScripts/script_green.sh"); //directorio puede modificarse o por parámetro???
    ...
    public static void sendingToPink(Session session){
		try{
			...
			messageMap.setStringProperty("fromPartyId", "domibus-blue"); //nodo inicial
			messageMap.setStringProperty("fromPartyType", "urn:oasis:names:tc:ebcore:partyid-type:unregistered");
			messageMap.setStringProperty("toPartyId", "domibus-pink"); //nodo destino
			messageMap.setStringProperty("toPartyType", "urn:oasis:names:tc:ebcore:partyid-type:unregistered");
			messageMap.setStringProperty("fromRole", "http://docs.oasis-open.org/ebxml-msg/ebms/v3.0/ns/core/200704/initiator");
			messageMap.setStringProperty("toRole", "http://docs.oasis-open.org/ebxml-msg/ebms/v3.0/ns/core/200704/responder");
			messageMap.setStringProperty("originalSender", "urn:oasis:names:tc:ebcore:partyid-type:unregistered:C1");
			...
			//Sending a file from one node to another works (testing script)
            File file = new File("/root/TFG/envioScripts/script_pink.sh"); //directorio puede modificarse o por parámetro???
			messageMap.setBytes("payload_1", FileManager.readFileAsBytes(file));
	...
	public static void main(String[] args){
		try{
			...
			Session sessionWGreen = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
			Session sessionWPink = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);

			//Calling function
			sendingToGreen(sessionWGreen);
			sendingToPink(sessionWPink);

			//Calling function
			listeningFromGreen(sessionWGreen);
			listeningFromPink(sessionWPink);
